...
function renderTasks() {
    ...
    sorted.forEach((task, index) => {
        ...
        item.innerHTML = `
            ...
            <button class="task-check" onclick="toggleTask(${task.id})" aria-label="Check">
                ${task.done ? checkIcon() : ''}
            </button>
            ...
            <div class="task-actions">
                <button class="task-edit" onclick="startEdit(${task.id})">${pencilIcon()}</button>
                <button class="task-delete" onclick="deleteTask(${task.id})">${trashIcon()}</button>
            </div>
        `;
...
// ── Icons (SVG) ───────────────────────────────────────────────
function checkIcon() { return `<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M2 7L5.5 10.5L12 3.5" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`; }
